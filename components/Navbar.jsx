  ...
  return (
    <nav className="relative border-b max-w-screen md:max-w-none z-50">
      <div className="container mx-auto flex justify-between items-center px-2 md:px-6 py-2 xl:py-4">
        <Link href={'/'}>
          <a className="text-xl md:text-2xl xl:text-3xl font-bold text-custom-black">MLYNEK.XYZ</a>
        </Link>
        <div className="flex space-x-4 items-center text-custom-black">
          ...
          className={`fixed left-0 flex justify-end w-full text-center divide-y md:divide-y-0 flex-col md:flex-row x md:relative md:space-x-4 md:items-center duration-500 ${isNavbarClosed ? '-bottom-full' : 'bottom-0 bg-black-opacity md:bg-white top-0 md:top-auto md:bottom-auto'}`}>
            ...
            <Link href={'/blog'}>
              <a className="text-2xl md:text-base font-semibold py-2 bg-white">Blog</a>
            </Link>
            <Link href={'/offer'}>
